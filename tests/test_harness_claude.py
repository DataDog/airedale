...
class TestClaudeAvailableTools:
    def test_unset_allows_all_builtins(self):
        runner = ClaudeRunner(allowed_builtin_tools=None)
        tools = runner._claude_available_tools()
        assert set(CLAUDE_BUILTIN_TOOLS) <= set(tools)

    def test_empty_list_allows_no_builtins(self):
        runner = ClaudeRunner(allowed_builtin_tools=[])
        assert runner._claude_available_tools() == []

    def test_explicit_list_is_exact(self):
        runner = ClaudeRunner(allowed_builtin_tools=["Read", "Grep"])
        assert runner._claude_available_tools() == ["Read", "Grep"]

    def test_mcp_tools_appended_with_allow_list(self):
        spec = McpServerSpec(name="apm", url="http://localhost:8000/mcp", tool_names=("search_apm",))
        runner = ClaudeRunner(allowed_builtin_tools=["Read"], mcp_servers=[spec])
        tools = runner._claude_available_tools()
        assert tools == ["Read", "mcp__apm__search_apm"]

    def test_mcp_server_without_tool_names_uses_prefix(self):
        spec = McpServerSpec(name="apm", url="http://localhost:8000/mcp")
        runner = ClaudeRunner(allowed_builtin_tools=[], mcp_servers=[spec])
