...
class TestBuildArgParser:
    ...
    def test_config_set(self):
        args = self.parser.parse_args(["--config", "exp.toml"])
        assert args.config == "exp.toml"

    def test_gateway_config_optional(self):
        args = self.parser.parse_args(["--config", "e.toml"])
        assert args.gateway_config is None

    def test_gateway_config_path(self):
        args = self.parser.parse_args(["--config", "e.toml", "--gateway-config", "gw.toml"])
        assert args.gateway_config == "gw.toml"

    def test_no_gateway_flag(self):
        args = self.parser.parse_args(["--config", "e.toml", "--no-gateway"])
        assert args.no_gateway is True

    def test_no_gateway_default_false(self):
        args = self.parser.parse_args(["--config", "e.toml"])
        assert args.no_gateway is False

    def test_model_filter(self):
        args = self.parser.parse_args(["--config", "e.toml", "--model", "anthropic/claude-3-haiku-20240307"])
    ...
    def test_scenario_filter(self):
        args = self.parser.parse_args(["--config", "e.toml", "--scenario", "default"])
        assert args.scenario == ["default"]

    def test_task_filter(self):
        args = self.parser.parse_args(["--config", "e.toml", "--task", "task1"])
        assert args.task == ["task1"]

    def test_runs_override(self):
        args = self.parser.parse_args(["--config", "e.toml", "--runs", "3"])
        assert args.runs == 3

    def test_judge_model_override(self):
        args = self.parser.parse_args(["--config", "e.toml", "--judge-model", "openai/gpt-4o"])
    ...
    def test_parallel_experiments_default(self):
        args = self.parser.parse_args(["--config", "e.toml"])
        assert args.parallel_experiments == 1

    def test_dry_run_flag(self):
        args = self.parser.parse_args(["--config", "e.toml", "--dry-run"])
    ...
    def test_no_agentless_flag(self):
        args = self.parser.parse_args(["--config", "e.toml", "--no-agentless"])
        assert args.agentless is False

    def test_no_progress_flag(self):
        args = self.parser.parse_args(["--config", "e.toml", "--no-progress"])
...
class TestMainDryRun:
    def test_dry_run_returns_zero(self, tmp_path, capsys):
        ...
        exit_code = main(["--config", str(config), "--dry-run"])
        assert exit_code == 0
    ...
    def test_dry_run_with_no_gateway(self, tmp_path, capsys):
        config = write_experiment_toml(tmp_path)
        exit_code = main(["--config", str(config), "--dry-run", "--no-gateway"])
        assert exit_code == 0
    ...
    def test_missing_config_file_returns_exit_2(self, tmp_path):
        exit_code = main(["--config", str(tmp_path / "nonexistent.toml"), "--dry-run"])
        assert exit_code == 2

    def test_invalid_toml_returns_exit_2(self, tmp_path):
        bad = tmp_path / "bad.toml"
        bad.write_text("this is not [ valid toml !!!")
        exit_code = main(["--config", str(bad), "--dry-run"])
        assert exit_code == 2
